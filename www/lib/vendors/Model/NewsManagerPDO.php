...
class NewsManagerPDO extends NewsManager
{
  ...
  public function getList($debut = -1, $limite = -1)
  {
    ...
    public function getUnique($id)
        {
        // Chargement de la news en cache
        $news = $this->loadNewsFromCache($id);

        // Si le cache est vide, la news est chargée depuis la base de données
        if (!isset($news)) {
            ...
            if ($news = $requete->fetch())
            {
                ...
                // Sauvegarde de la News en Cache
                $this->saveNewsToCache($news);
            }
        }
  ...
  protected function modify(News $news)
  {
    ...
    // Suppression du cache
    $this->clearNewsFromCache($news->id());
  }
}
